        ...
        for (let i = 0; i < coordtab.length; i++) {
            for (let j = 0; j < this.tabOfBoats.length; j++) {
                if (dragBoat.id == this.tabOfBoats[j].id)
                    continue;
                if (coordtab[i].x >= this.tabOfBoats[j].privateZone[0].x && this.tabOfBoats[j].privateZone[1].x >= coordtab[i].x && coordtab[i].y >= this.tabOfBoats[j].privateZone[0].y && this.tabOfBoats[j].privateZone[1].y >= coordtab[i].y) {
                    return true;
                }
            }
        }
